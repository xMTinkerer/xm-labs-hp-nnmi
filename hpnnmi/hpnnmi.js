...
function apia_http( httpRequestProperties, httpResponse )
{
  ...
  if ( requestBody == null )
  {
    ...
    var errText = "Received unrecognize Request, no notification was created. (Suppressed)";
    log.debug( preamble + errText );

    httpResponse.setBodyString( JSON.stringify( {"error": errText } ) );
  ...
  if ( incidentName.length() <= 0)
  {
    ...
    var errText = "No xMatters Event was generated as the required node <arg0><name/></arg0> was not found in the request body."
    httpResponse.setBodyString( JSON.stringify( {"error": errText } ) );
    httpResponse.setHeader( new Header("Content-Type", "application/json" ) );
...
function handleResponse( msg )
{
  ...
  var messageText = "Event [" + msg.eventidentifier + "]: " + msg.recipient +
    " responded with " + msg.response + " on " + msg.device + ( msg.annotation == "null" ? null : ' Notes: ' + msg.annotation );

  var additionalTokens = msg.additionalTokens;
  var nnmid            = additionalTokens.id;
  var incidentId       = additionalTokens.id;
  ...
  {
      var errorMessage = "Unknown response '" + response + "' ignoring.";
...
function handleDeliveryStatus( msg )
{
  ...
  var messageText =  "Event [" + msg.eventidentifier + "]: " + msg.deliverystatus +
    " to " + msg.recipient + " on " + msg.device;
...
function handleEventStatus( msg )
{
  ...
  var messageText =  "Event [" + msg.eventidentifier + "]: Status has changed to " + msg.status;
